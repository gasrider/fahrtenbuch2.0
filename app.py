...
def save_hu_corrections(username, corrections_list):
    """Speichert HU-Korrekturen in die Tabelle 'hu_corrections' (Delete + Insert)."""
    try:
        ...
        for c in corrections_list:
            clean_rows.append({
                "username": username,
                "fahrzeug_id": int(c.get("fahrzeug_id")) if c.get("fahrzeug_id") is not None else None,
                "datum": str(c["datum"]) if c.get("datum") else None,
                "km_at_hu": _safe_int(c.get("km_at_hu")),
                "werkstattort": str(c.get("werkstattort", "")).strip(),
                "stopps_vor_hu": ",".join(c.get("stopps_vor_hu", [])),
                "stopps_nach_hu": ",".join(c.get("stopps_nach_hu", [])),
            })
        for i in range(0, len(clean_rows), 500):
            supabase.table("hu_corrections").insert(clean_rows[i:i+500]).execute()
        return True
    except Exception as e:
        st.error(f"Fehler beim Speichern der HU-Korrekturen: {getattr(e, 'message', str(e))}")
        return False

def load_hu_corrections(username, fahrzeuge_df=None):
    """Lädt HU-Korrekturen aus der Tabelle 'hu_corrections'. Loest fahrzeug_id in Namen auf."""
    try:
        response = supabase.table("hu_corrections").select("*").eq("username", username).execute()
        if response.data:
            # Mapping fahrzeug_id -> bezeichnung aufbauen
            id_to_name = {}
            ...
            for r in response.data:
                fz_id = r.get("fahrzeug_id")
                fz_name = id_to_name.get(int(fz_id), "") if fz_id is not None else ""
                rows.append({
                    "Fahrzeug": fz_name,
...
def get_valid_fahrzeug_optionen(fahrzeuge_df):
    """Liefert ein Dict {bezeichnung: id} nur für Fahrzeuge mit gültigem Namen."""
    valid = {}
    for _, row in fahrzeuge_df.iterrows():
...
def auto_correct_red_flags(generated_months_data, fahrzeuge_df, max_avg_speed=100):
    ...
    2. Überschüssige km werden auf das GANZE JAHR verteilt:
       - ~60% als kleine km_d-Zuschläge auf dienstliche Tage (max +3 km/Tag)
       - ~40% als kleine km_p-Zuschläge auf verschiedene Tage (max +5 km/Tag)
       → Pro Tag nur wenige km mehr, fällt bei Prüfung nicht auf
    ...
    # Phase 2: Verteilungs-Ziele sammeln (Flag-Tage ausschließen)
    flag_indices = set()
    for monat_key, data in generated_months_data.items():
        df = data["data"]
        for idx, row in df.iterrows():
            ...
            if total_km > 0:
                ...
                if dauer_min > 0:
                    ...
                    if geschw > max_avg_speed:
                        flag_indices.add((monat_key, idx))
    
    dienstlich_ziele = []  # Für km_d: Tage mit km_d > 0, realistisch
    privat_ziele = []      # Für km_p: alle Tage mit km_d > 0 oder km_p > 0
    
    for monat_key, data in generated_months_data.items():
        df = data["data"]
        for idx, row in df.iterrows():
            if (monat_key, idx) in flag_indices:
                continue
            km_d = _safe_int(row.get("km_d"))
            km_p = _safe_int(row.get("km_p"))
            total_km = km_d + km_p
    ...
    return generated_months_data, korrekturen, verteilte_km
...
with st.expander("⛱ Urlaubswochen (optional)"):
    ...
    if anzahl_wochen > 0:
        ...
        with colU4:
            fahrzeug_optionen = get_valid_fahrzeug_optionen(fahrzeuge_df)
            urlaub_fahrzeug = st.selectbox("Fahrzeug für private Urlaubs-KM", options=list(fahrzeug_optionen.keys()), key="urlaub_fahrzeug", disabled=not fahrzeug_optionen)
...
fahrzeug_namen_liste = [str(b).strip() for b in fahrzeuge_df['bezeichnung'].dropna().tolist() if str(b).strip() and str(b).strip().lower() not in ('none', 'nan')] if not fahrzeuge_df.empty else []
with hfz_col1:
...
if gen_btn:
    ...
    if st.session_state.get("generated_months_data") and len(st.session_state["generated_months_data"]) == 12:
        ...
        if anz_korr > 0:
            ...
            st.success(f"🔧 Auto-Korrektur: {anz_korr} unrealistische Fahrt(en) korrigiert, {verteilte_km} km über das Jahr verteilt (dienstl. + privat).")
            st.rerun()
        ...
        if _speed_flags:
            with st.spinner("🔧 Korrigiere unrealistische Geschwindigkeiten..."):
                ...
                if anz_korr > 0:
                    ...
                    save_fahrten_to_db(user, corrected_data)
                    st.success(f"🔧 Auto-Korrektur: {anz_korr} Fahrt(en) korrigiert, {verteilte_km} km über das Jahr verteilt (dienstl. + privat).")
...
if st.session_state.get("generated_months_data") and len(st.session_state["generated_months_data"]) == 12:
    ...
    with st.expander("Korrekturen vornehmen"):
        saved_hu = load_hu_corrections(user, fahrzeuge_df)
        hu_events_df = pd.DataFrame(saved_hu, columns=["Fahrzeug", "Datum der HU", "Kilometerstand bei HU", "Werkstattort (Dropdown)", "Kundenstopps vor HU (mehrere möglich)", "Kundenstopps nach HU (mehrere möglich)"]) if saved_hu else pd.DataFrame(columns=["Fahrzeug", "Datum der HU", "Kilometerstand bei HU", "Werkstattort (Dropdown)", "Kundenstopps vor HU (mehrere möglich)", "Kundenstopps nach HU (mehrere möglich)"])
        fahrzeug_optionen = get_valid_fahrzeug_optionen(fahrzeuge_df)
            ...
            if edited_hu_events.empty: st.warning("Bitte gib mindestens eine Korrektur ein.")
                ...
                if not correction_data: st.error("Ungültige Eingabe. Bitte stelle sicher, dass alle Pflichtfelder ausgefüllt sind.")
                    ...
                    with st.spinner("Wende Korrekturen an..."):
                            ...
                            if not trips_before_hu.empty:
                                ...
                                if calculated_start_km_hu_day > start_km:
                                    ...
                                    def _scale_dauer(dauer_str, factor):
                                        ...
                                        except Exception: return dauer_str
                                    def _recalc_ankunft(row):
                                        ...
                                        except Exception: return row['ank']
                        ...
                        save_fahrten_to_db(user, st.session_state["generated_months_data"])
                        save_hu_corrections(user, correction_data)
